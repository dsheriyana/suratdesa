...
<page orientation="portrait" format="210x330" style="font-size: 10pt">
	...
	<p style="text-align: center;">
		<strong style="font-size: 12pt;">FORMULIR KETERANGAN PINDAH DATANG WNI</strong><br>
		<b>Antar Desa / Kelurahan Dalam Satu Kecamatan<br></b>
		<b>No. <?= $no_skpdw; ?></b>
	</p>
	...
	<table class="disdukcapil">
		...
		<tr>
			<td>4.</td>
			<td>Nama Kepala Keluarga</td>
			<td colspan=22 class="kotak"><?= $nama_kepala; ?></td>
		</tr>
		<tr>
			<td>5.</td>
			<td>Tanggal Kedatangan</td>
				<?php
					$date = date($tgl_kedatangan);
	...
	<table class="pengikut">
		...
		for ($i = 0; $i < MAX_PINDAH; $i++) :
			$nomor = $i + 1;
			if ($i < count($nik_pengikut)):
				$nik_p = trim($nik_pengikut[$i], "'");
	...
	<table class="ttd">
		...
		<tr style="font-size: 25mm; line-height: normal;">
			<td>&nbsp;</td>
			<td>&nbsp;</td>
			<td>&nbsp;</td>
		</tr>
...
<form id="formSubmit">
		...
		<input type="hidden" name="jumlah_ikut[]" value="<?= $nik_p; ?>">
		<input type="hidden" name="nik_penduduk[]" value="<?= $nik_p; ?>">
		<input type="hidden" name="ktp_berlaku[]" value="1">
		<input type="hidden" name="nama_penduduk[]" value="1">
	...
	<input type="hidden" name="no_skpdw" value="<?= $no_skpdw; ?>">
...
<script>
	...
	const simpan_data = async (formData) => {
		let url = `${base_url}surat/reviewSKPDW`;
